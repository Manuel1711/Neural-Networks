...
plot  "data_sec0611ntk1000.dat" every 200 using ($1):($2*90**(0.)):($3) t 'Dataset 3' w yerrorlines pt 5 ps 1 lw 3. lt 8 dt 1 pi 100,\
      ".dat" using ($1):($2*50**(0.)) t '' w l lw 3. lt rgb 'blue' dt 5,\
      "Output_ens_sigma_0.01 (1).ttxt" using ($1):($2*90**(0.)/3.14):($3/3.14) t 'BG' w yerrorlines pt 3 ps 1 lw 3. dt 3 lt 1,\
      "test.ddat" using ($1):(($2)*30**(0.)) t 'smear' w l lw 3. lt rgb 'dark-green' dt 2,\
      "data_secntk20_2heavyran+2ranscal.dat" every 200 using ($1):($2*90**(0.)):($3) t 'Dataset 2' w yerrorlines pt 1 ps 1 lw 3. dt 5 pi 100 lt 6,\
      "data_secntk20_2heavyran.dat" every 200 using ($1):($2*70**(0.)):($3) t 'Dataset 1' w yerrorlines pt 7 ps 1 lw 1. lt rgb 'red' dt 4 pi 100,\
